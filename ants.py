...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        ans = None
        place_traverse = self.place
        place_index = 0
        while place_traverse and not (place_traverse.is_hive):
            bee = random_bee(place_traverse.bees)
            if bee and  self.lower_bound <= place_index <= self.upper_bound:  
                ans = bee
                break
            
            place_traverse = place_traverse.entrance
            place_index += 1
        return ans
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for i in range(len(bee_lst)-1, -1, -1):
            self.place.bees[i].reduce_health( amount + (self.damage if self.health - amount <= 0 else 0))
...
class BodyguardAnt(ContainerAnt):
    ...
    def __init__(self,health= 2, *args, **kargs):
        return super().__init__(health, *args, **kargs) 
    # END Problem 8c
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        place_indx = self.place.exit
        while place_indx:
            if hasattr(place_indx.ant, 'ant_contained'):
                Ant.double(place_indx.ant)
                
                if place_indx.ant.ant_contained:
                    Ant.double(place_indx.ant.ant_contained)
            elif place_indx.ant:
                Ant.double(place_indx.ant)
            
            place_indx = place_indx.exit
        super().action(gamestate)
        # END Problem 12
